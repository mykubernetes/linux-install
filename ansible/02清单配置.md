...
| `ansible_become` | 允许特权升级，2.2版本后的特性。等同于`ansible_sudo`、`ansible_su`。 |  |
| `ansible_become_method` | 设置特权提升的方法，2.2版本后的特性。比如`sudo`。 |  |
| `ansible_become_user` | 设置特权提升的用户，2.2版本后的特性。等同于`ansible_sudo_user`，`ansible_su_user` |  |
| `ansible_become_password` | 设置特权用户的密码,2.2版本后的特性。等同于`ansible_sudo_password`，`ansible_su_password` |  |
